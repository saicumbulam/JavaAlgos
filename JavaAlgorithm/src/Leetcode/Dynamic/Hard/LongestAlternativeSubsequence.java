...
public class LongestAlternativeSubsequence {
    ...
    private static int findLASLengthRecursive(int[] nums,
                                              int previousIndex,
                                              int currentIndex, boolean isAsc) {
        ...
        if(isAsc) {
            if(previousIndex == -1 || nums[previousIndex] < nums[currentIndex])
                c1 = 1 + findLASLengthRecursive(nums, currentIndex,
                        currentIndex+1, !isAsc);
        ...
        int c2 = findLASLengthRecursive(nums, previousIndex,
                currentIndex+1, isAsc);
